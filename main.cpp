...
void win_resize (GLFWwindow * win, int width, int height)
{
    ...
    gluPerspective(60.0, static_cast<float> (width*5.0)/ static_cast<float> (height*5.0), 1, 40);
}
...
void win_refresh (GLFWwindow *win) {
    ...
    sphere_ufo.render(false);
    glPopMatrix();
    glTranslatef(0, 0 ,-0.22);
    ufo_one.render(false);
    glPopMatrix();

    // move wheel off to side for now
    glPushMatrix();
    glTranslatef(0, 2 ,1);
    wheels.render(false);
    glPopMatrix();
...
void make_model() {
    int N = 6;		// determines sphere smoothness
    int M = 3;
    int lamp_elbow_angle = 50;
    int light_smoothness = 6;
    int island_elbow_angle = 50;
    int grass_parameter = 1;

    sphere_ufo.build ((void *)&N);
    ufo_one.build(nullptr);
    wheels.build(nullptr);
    lamp.build((void *)&lamp_elbow_angle);
    light.build((void *)&light_smoothness);
    island.build((void *)&island_elbow_angle);
    grass.build((void *)&grass_parameter);
}
